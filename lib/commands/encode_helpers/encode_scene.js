...
function encodeSceneFile(file, scene) {
    ...
    encoders.forEach(encoder => {
        if (encoder.extension == file.extension) {
            ...
            const outputFile = path_1.join(encode_1.outputDir, scene.name, "h-stripes", file.name + ".h-stripe");
...
function mergeEncodedScene(scene) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        reduced.include_directive = applySceneReplaces(encoder_templates_1.template_scene_include, scene);
        files.forEach(file => {
