...
function scan(inputFolder, outputFile, mode = "run") {
    if (mode == "help")
        ...
        let results = root_folders(inputFolder, vflag);
...
exports.scan = scan;
function list(folder, name, verbose) {
    let files = [];
    let directories = [];
    fs_1.readdirSync(folder, { withFileTypes: true })
        ...
        if (entry.isFile()) {
            ...
            directories.push(list(folder + "/" + entry.name, entry.name, verbose));
        }
    });
    return { files: files, path: folder, name: name, directories: directories };
}
function root_folders(root, verbose) {
    let shared = [];
    let scenes = [];
    let unused = [];
    fs_1.readdirSync(root, { withFileTypes: true })
        ...
        let data = list(root + "/" + dir, dir, verbose);
