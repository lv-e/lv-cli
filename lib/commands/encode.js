...
function encode(input, output, mode) {
    return __awaiter(this, void 0, void 0, function* () {
        if (mode == "help")
            ...
            let promises = yield data.scenes.map((scene) => __awaiter(this, void 0, void 0, function* () { return yield encodeScene(scene); }));
...
function encodeSharedDir(dir) {
    helpers_1.log(vflag, chalk.blue("encoding shared folder: ") + chalk.cyan(dir.name));
...
function encodeSharedFile(file) {
    encoders.forEach(encoder => {
        if (encoder.extension == file.extension) {
            helpers_1.log(vflag, "encoding shared file: " + file.name + " with " + encoder.npm_module);
            const outputFile = path_1.join(outputDir, "shared", "h-stripes", file.name + ".h-stripe");
            shelljs_1.default.exec(encoder.cli_command + " -i " + file.path + " -o " + outputFile);
        }
    });
}
function encodeMainFiles(encodedScenes) {
    helpers_1.log(vflag, chalk.blue("generating main.c file"));
...
function encodeProject(file) {
    ...
    encoders.forEach(encoder => {
        if (encoder.extension == file.extension) {
            ...
            helpers_1.log(vflag, chalk.blue("encoding project: ") + chalk.cyan(file.name));
            helpers_1.log(vflag, "using " + encoder.npm_module);
            shelljs_1.default.exec(encoder.cli_command + " -i " + file.path + " -o " + outputFile);
        }
    });
}
function encodeScene(scene) {
    return __awaiter(this, void 0, void 0, function* () {
        helpers_1.log(vflag, chalk.blue("encoding: ") + chalk.cyan(scene.name));
...
function mergeEncoded(scene) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        reduced.globals = applySceneReplaces(encoder_templates_1.template_scene_include, scene);
        files.forEach(file => {
